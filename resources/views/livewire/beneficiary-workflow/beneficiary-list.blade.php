        ...
        <div class="flex flex-col md:flex-row md:items-center gap-2.5 w-full md:w-1/2">
            <div class="relative rounded-md shadow-sm">
                ...
                <input type="text" name="search" id="search" wire:model.live="searchItem"
                    class="focus:ring-indigo-500 focus:border-indigo-500 district w-full px-10 sm:text-sm border-gray-300 rounded-md"
                    placeholder="Search District Name" autocomplete="off">
            ...
            <span class="flex text-sm font-medium text-gray-500">Total Available: {{ $districts->total() }}</span>
        </div>
    </div>

    <div class="flex flex-col gap-2">
        <div class="flex px-4 py-2.5 bg-gray-700 border rounded shadow">
            <div class="flex-1 grid grid-rows-1 md:grid-cols-5 gap-4 text-sm font-semibold text-gray-50">
                    ...
                    <div class="flex-1 grid grid-rows-1 md:grid-cols-5 bg-white p-4 border-l-2 rounded px-2 py-1">
                        <div class="sm:hidden flex w-full">
                            <div class="flex flex-col justify-center text-gray-500 w-4/5">
                                <span class="text-sm font-bold hover:text-indigo-400 mt-1">{{ $district->name }}</span>
                            </div>
                            <div class="flex gap-2 w-1/5">
                                {{-- @can('update district') --}}
                                <button
                                    class="bg-blue-50 text-blue-800 p-1.5 rounded-full border border-blue-300 mb-2 mt-2"
                                    wire:click="openEditForm('{{$district->id}}')">
                                ...
                                {{-- @can('delete district') --}}
                                <button type="button" onclick="confirmDeleteDistrict('{{ $district->id }}')"
                        ...
                        <div class="hidden sm:flex flex-col justify-center text-gray-500">
                            <span class="text-sm font-bold mt-1">{{ $district->name }}</span>
                        </div>
                        <div class="hidden sm:flex flex-col justify-center text-gray-500 ml-4">
                            <span class="text-xs font-bold mt-1">{{ $district->ref_code }}</span>
                        </div>
                        <div class="hidden sm:flex flex-col justify-center text-gray-500 ml-4">
                            <span class="text-xs font-semibold mt-1">{{ $district->lg_code }}</span>
                        </div>
                        <div class="hidden sm:flex flex-col justify-center text-gray-500">
                            <span class="text-sm font-bold mt-1">{{ $district->state->name }}</span>
                        </div>
    ...
    @once
        @push('scripts')
            <script>
                ...
                const confirmDeleteDistrict = (id) => {
                    Swal.fire({
                        title: 'Are you sure?',
                        text: "Confirm if you want to delete the District",
                        icon: 'warning',
                            ...
                            @this.deleteDistrict(id).then(
                                (response) => {
                                    if (response.status == 200)
                                        ...
                                        Swal.fire({
                                            title: 'Error in Deleting District',
                                            text: "District can not be deleted",
                                            icon: 'error',
                                        });
                                },
                            );
